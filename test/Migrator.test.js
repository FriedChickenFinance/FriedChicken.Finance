...
contract('Migrator', ([alice, bob, dev, minter]) => {
    beforeEach(async () => {
        ...
        this.colonel = await Colonel.new(this.chicken.address, dev, '1000', '0', '100000', { from: alice });
        this.migrator = await Migrator.new(this.colonel.address, this.factory1.address, this.factory2.address, '0');
        await this.chicken.transferOwnership(this.colonel.address, { from: alice });
        await this.colonel.add('100', this.lp1.address, true, { from: alice });
    });
    ...
    it('should do the migration successfully', async () => {
        ...
        await this.lp1.sync();
        await this.lp1.approve(this.colonel.address, '100000000000', { from: minter });
        await this.colonel.deposit('0', '2000000', { from: minter });
        assert.equal((await this.lp1.balanceOf(this.colonel.address)).valueOf(), '2000000');
        await expectRevert(this.colonel.migrate(0), 'migrate: no migrator');
        await this.colonel.setMigrator(this.migrator.address, { from: alice });
        await expectRevert(this.colonel.migrate(0), 'migrate: bad');
        await this.factory2.setMigrator(this.migrator.address, { from: alice });
        await this.colonel.migrate(0);
        assert.equal((await this.lp1.balanceOf(this.colonel.address)).valueOf(), '0');
        assert.equal((await this.lp2.balanceOf(this.colonel.address)).valueOf(), '2000000');
        await this.colonel.withdraw('0', '2000000', { from: minter });
