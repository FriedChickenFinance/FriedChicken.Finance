...
const ChickenDredger = artifacts.require('ChickenDredger');
const MockERC20 = artifacts.require('MockERC20');
const UniswapV2Pair = artifacts.require('UniswapV2Pair');
const UniswapV2Factory = artifacts.require('UniswapV2Factory');

contract('ChickenDredger', ([alice, bar, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.chicken = await ChickenToken.new({ from: alice });
        await this.chicken.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await ChickenDredger.new(this.factory.address, bar, this.chicken.address, this.weth.address);
        this.chickenWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.chicken.address)).logs[0].args.pair);
    ...
    it('should make CHICKs successfully', async () => {
        await this.factory.setFeeTo(this.maker.address, { from: alice });
        await this.weth.transfer(this.chickenWETH.address, '10000000', { from: minter });
        await this.chicken.transfer(this.chickenWETH.address, '10000000', { from: minter });
        await this.chickenWETH.mint(minter);
        ...
        // After calling convert, bar should have CHICK value at ~1/6 of revenue
        await this.maker.convert(this.token1.address, this.token2.address);
        assert.equal((await this.chicken.balanceOf(bar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for CHICK-ETH pair
        await this.chicken.transfer(this.chickenWETH.address, '100000', { from: minter });
        await this.weth.transfer(this.chickenWETH.address, '100000', { from: minter });
        await this.chickenWETH.sync();
        await this.chicken.transfer(this.chickenWETH.address, '10000000', { from: minter });
        await this.weth.transfer(this.chickenWETH.address, '10000000', { from: minter });
        await this.chickenWETH.mint(minter);
        assert.equal((await this.chickenWETH.balanceOf(this.maker.address)).valueOf(), '16537');
        await this.maker.convert(this.chicken.address, this.weth.address);
        assert.equal((await this.chicken.balanceOf(bar)).valueOf(), '66249');
        assert.equal((await this.chickenWETH.balanceOf(this.maker.address)).valueOf(), '0');
    });
});