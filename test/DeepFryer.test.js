...
const DeepFryer = artifacts.require('DeepFryer');

contract('ChickenToken', ([alice, bob, carol]) => {
    beforeEach(async () => {
        this.chicken = await ChickenToken.new({ from: alice });
        this.bar = await DeepFryer.new(this.chicken.address, { from: alice });
    ...
    it('should not allow enter if not enough approve', async () => {
        ...
        await this.chicken.approve(this.bar.address, '50', { from: alice });
        await expectRevert(
            this.bar.enter('100', { from: alice }),
            'ERC20: transfer amount exceeds allowance',
        );
        await this.chicken.approve(this.bar.address, '100', { from: alice });
    ...
    it('should work with more than one participant', async () => {
        await this.chicken.approve(this.bar.address, '100', { from: alice });
        await this.chicken.approve(this.bar.address, '100', { from: bob });
        ...
        assert.equal((await this.chicken.balanceOf(this.bar.address)).valueOf(), '30');
        // DeepFryer get 20 more CHICKs from an external source.
        await this.chicken.transfer(this.bar.address, '20', { from: carol });
        // Alice deposits 10 more CHICKs. She should receive 10*30/50 = 6 shares.
        ...
        assert.equal((await this.chicken.balanceOf(this.bar.address)).valueOf(), '52');
