...
contract('Governor', ([alice, minter, dev]) => {
    it('should work', async () => {
        ...
        this.colonel = await Colonel.new(this.chicken.address, dev, '100', '0', '0', { from: alice });
        await this.chicken.transferOwnership(this.colonel.address, { from: alice });
        this.lp = await MockERC20.new('LPToken', 'LP', '10000000000', { from: minter });
        this.lp2 = await MockERC20.new('LPToken2', 'LP2', '10000000000', { from: minter });
        await this.colonel.add('100', this.lp.address, true, { from: alice });
        await this.lp.approve(this.colonel.address, '1000', { from: minter });
        await this.colonel.deposit(0, '100', { from: minter });
        // Perform another deposit to make sure some CHICKs are minted in that 1 block.
        await this.colonel.deposit(0, '100', { from: minter });
        assert.equal((await this.chicken.totalSupply()).valueOf(), '110');
        ...
        await this.colonel.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.colonel.add('100', this.lp2.address, true, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.gov.propose(
                [this.colonel.address], ['0'], ['add(uint256,address,bool)'],
        ...
        assert.equal((await this.colonel.poolLength()).valueOf(), '2');
    });
});
