...
contract ChickenDredger {
    ...
    constructor(IUniswapV2Factory _factory, address _bar, address _chicken, address _weth) public {
        factory = _factory;
        chicken = _chicken;
        bar = _bar;
        weth = _weth;
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        if (token == weth) {
            uint amount = IERC20(token).balanceOf(address(this));
            IERC20(token).transfer(factory.getPair(weth, chicken), amount);
            return amount;
        }
        ...
        pair.swap(amount0Out, amount1Out, factory.getPair(weth, chicken), new bytes(0));
        return amountOut;
    }

    function _toCHICK(uint256 amountIn) internal {
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(weth, chicken));
        (uint reserve0, uint reserve1,) = pair.getReserves();
        address token0 = pair.token0();
